...
export default function SignIn() {
  ...
  const [errorMessage, setErrorMessaje] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.password || !formData.email) {
      return setErrorMessaje("Please fill out the fields");
    }
    try {
      setLoading(true);
      ...
      if (data.success === false) {
        return setErrorMessaje(data.message);
      }
      setLoading(false);
      if (res.ok) {
        navigate("/");
      ...
      <div className="flex p-3 max-w-3xl mx-auto flex-col md:flex-row md:items-center gap-5">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm mt-5">
            This is a demo project. you can sign in with google or your e-mail
            and password.
          </p>
        </div>
        {/*Derecha*/}
